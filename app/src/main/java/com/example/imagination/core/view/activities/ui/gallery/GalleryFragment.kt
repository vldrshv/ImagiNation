...
class GalleryFragment : MvpAppCompatFragment(), PhotoView {
    ...
    private fun initRecycleView() {
        ...
        photoRecyclerView.addOnScrollListener(recyclerViewPresenter.recyclerViewEndPageListener())
        photoRecyclerView.addItemDecoration(ItemDecoration(4))

        val size = Point()
        activity!!.windowManager.defaultDisplay.getSize(size)
        photoRecyclerView.adapter = MyAdapter(recyclerViewPresenter, size, isPortraitOrientation())
    }
